...
#data_model = read.csv('西苑数据-判别分析原始变量.csv',fileEncoding = 'GBK',stringsAsFactors = F)
model_file_path = './model/cmccm.rda'
cmccmbuildmodel = function(data_model, model_file_path){
...
cmccmpredfunc = function(input){
  ...
  New_patient[,"BRAF突变型"] = New_patient[,"f2"]
  New_patient[,"RAS突变型"] = New_patient[,"f3"]
  New_patient[,"KPS评分"] = sqrt(New_patient[,"f4"])
  New_patient[,"肝转移"] = New_patient[,"f5"]
  New_patient[,"肺转移"] = New_patient[,"f6"]
  New_patient[,"肺"] = New_patient[,"f7"]
  New_patient[,"肾"] = New_patient[,"f8"]
  New_patient[,"肝"] = New_patient[,"f9"]
...
testpredict = function(){
  New_patient = data.frame('f4'=70,'f5'=1,'f6'=0,'f7'=0,"f8"=1,"f9"=1,"f10"=0,
                           "f11"=24,"f12"=7,"f13"=11,
                           "f1"=0,"f2"=0,"f3"=1)
  print(cmccmpredfunc(New_patient))
  New_patient = data.frame('f4'=sqrt(70),'f5'=1,'f6'=0,'f7'=0,"f8"=1,"f9"=1,"f10"=0,
